...
class CNW_NewsCustomPostType {
	public function __construct() {
		add_action( 'init', array( $this, 'cnw_cpt_init' ) );
	}

	/**
	 * Register News custom post type
	 */
	function cnw_cpt_init() {
		$labels = array(
			'name'               => _x( 'News', 'post type general name', 'text_domain' ),
			'singular_name'      => _x( 'News', 'post type singular name', 'text_domain' ),
			'menu_name'          => _x( 'News', 'admin menu', 'text_domain' ),
			'name_admin_bar'     => _x( 'News', 'add new on admin bar', 'text_domain' ),
		...
		$args = array(
			'labels'             => $labels,
			'description'        => __( 'Custom Post for News', 'text_domain' ),
			'public'             => true,
